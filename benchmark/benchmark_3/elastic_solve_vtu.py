...
which re-invokes this script once per file as its own subprocess.
...
def bench_one(path: str, label: str, formulation: str, scheme: str) -> dict:
    ...
    return {
        ...
        "output": os.path.join(OUT_DIR, stem + ".xdmf"),
    }
...
def main():
    ...
    print(f"Found {len(paths)} .vtu file(s) in {args.data_dir}, "
          f"{len(SOLVER_CONFIGS)} solver config(s) each"
          + ("  (--isolate: one subprocess per run)" if args.isolate else ""))
    ...
    for path in paths:
        for label, formulation, scheme in SOLVER_CONFIGS:
            if args.isolate:
                ...
                r = bench_one(path, label, formulation, scheme)
            results.append(r)
            print_row(r)
