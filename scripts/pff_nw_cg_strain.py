...
Combines the multi-phase RVE setup of elastic_nw_cg_strain.py with the
AT2 phase-field fracture solver from benchmark_pff_tension.py.

Reference: Schneider & Kästner (2025)  https://doi.org/10.1111/ffe.14553

Geometry   : square composite RVE generated by make_square_composite_rve
Materials  : epoxy matrix  +  transversely isotropic carbon fibre
Units      : MPa, µm  (so Gc in MPa·µm = J/m², l₀ in µm)
Load       : uniaxial strain ramp ε₁₁ → 1×10⁻³ with adaptive stepping

PFF scheme (per staggered iteration)
--------------------------------------
  1. Degrade stiffness       C_eff = g(d) · C_field
  2. Mechanical CG solve     (ε, σ) = solve_elastic(C_eff, G_glob, ε̄)
  3. Crack driving force     ψ⁺ from Miehe spectral split (undegraded C)
  4. Hybrid history update   H = max(H_prev, ψ⁺)  if d ≥ dthres,
                             H = ψ⁺               otherwise
  5. Helmholtz CG solve      d = solve_helmholtz_cg(H, η, Δt)
  6. Staggered convergence   |Δd|∞ < εa  OR  |Δd|∞/|d|∞ < εr
...
Output
------
    output/rve_pff_loadsteps.h5
    output/rve_pff_loadsteps.xdmf
"""
...
nz       = 10       # voxels in z (out-of-plane)
...
lam_vox, mu_vox = lame_from_C_field(C_field)   # undegraded — fixed throughout

# ── Reference medium (Voigt average of transverse fibre + matrix) ─────────────
...
# ── PFF parameters (MPa, µm) ──────────────────────────────────────────────────
# Units: Gc [MPa·µm = J/m²],  l₀ [µm]
# Epoxy matrix fracture energy ≈ 100 J/m²; l₀ > 4 × voxel size = 0.4 µm.
# Fibre phase uses the same Gc/l₀ as a first approximation.

l0     = 1.0      # µm  — phase-field length scale  (≥ 4 × vox_um = 0.4 µm)
Gc     = 100.0    # J/m² = MPa·µm  (epoxy fracture energy)
dthres = 0.95     # hybrid irreversibility threshold (Schneider & Kästner 2025)
eta    = 1e-6     # numerical viscosity (same as kres, paper Figure 3b)
...
# ── Staggered scheme parameters ───────────────────────────────────────────────

toler_st_abs = 1e-2    # εa — absolute staggered convergence
toler_st_rel = 1e-3    # εr — relative staggered convergence
maxiter_st   = 200
toler_helm   = 1e-3
maxiter_helm = 300
...
# ── Initial damage and history fields ─────────────────────────────────────────
...
with IncrementalWriter(
    ...
    while t < t_end and step < max_steps:
        ...
        # ── cutback loop (mechanical non-convergence only) ────────────────────
        for attempt in range(max_cutbacks + 1):
            ...
            for iter_st in range(1, maxiter_st + 1):
                ...
                H_st = update_history_hybrid(H_st, psi_pos, d_st, dthres=dthres)
        ...
        print(
            f"  step {step:3d}  t={t:.4f}  ε₁₁={float(state.strain_ave[0,0]):.2e}  "
