...
eps_loc, sigma_loc, delta, info = solve_elastic(
    n, C_field, G_glob, eps_bar, toler_lin=1e-10, maxiter=2000
)

print(f"CG exit flag : {info}  (0 = converged)")
...
print()
if float(err_sigma / norm_sigma) < 1e-6 and (info == 0 or info is None):
...
u_grid      = compute_displacement(eps_loc, eps_bar, xi_flat, n, dx)  # (*n, 3)
