...
Usage
-----
    python scripts/simulation/pff_nw_cg_strain.py configs/pff_rve.yaml
...
print(f"Reference: lam0={lam0/1e3:.4f} GPa  mu0={mu0/1e3:.4f} GPa")

slv     = cfg["solver"]
xi_flat = build_freq_grid(n, L)
G_glob  = build_green_operator(
    xi_flat, lam0, mu0,
    scheme=slv.get("scheme", "rotated"), dx=dx,
)
...
with IncrementalWriter(f"{output}/{jobname}", grid_shape=n, grid_spacing=dx) as w:
    ...
    while t < t_end and step < max_steps:
        ...
        u_grid     = compute_displacement(state.strain_loc, eps_bar_i, n, dx)
