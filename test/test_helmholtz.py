...
Tests ``solve_damage_helmholtz_cg`` in isolation — no mechanical solve needed.
...
Homogeneous Gc only, matching ``solve_damage_helmholtz_cg``'s current scope
(k defaults to 0.0 -- no residual-stiffness scaling needed here, this test
never varies k_res) -- add a heterogeneous-Gc check here if/when
solvers/elliptic/scalar.py grows that variant.
...
d_out, conv_h = solve_damage_helmholtz_cg(L_uniform, xi_flat, n, l0, Gc, d_prev,
...
d_out, conv_h = solve_damage_helmholtz_cg(L_crack, xi_flat, n, l0, Gc, d_prev,
...
L_zero    = jnp.zeros((Nv,))
d_irrev, _ = solve_damage_helmholtz_cg(L_zero, xi_flat, n, l0, Gc, d_out,
                                        toler_cg=1e-8, maxiter=500)
...
for l0 in l0_values:
    ...
    d_out, conv_h = solve_damage_helmholtz_cg(L_field, xi_flat, n, l0, Gc, d_prev,
                                               toler_cg=1e-8, maxiter=500)
    d_np = np.asarray(d_out).reshape(n)
...
# Not asserted: half_width is measured off a fixed, fairly coarse dx=0.2mm
# grid (r values quantized in 0.2mm steps), so at small l0 (e.g. l0=0.5mm,
# where the whole decay spans ~2 grid cells) the 1/e-crossing measurement is
# dominated by quantization noise rather than solver error -- this is a
# qualitative sanity check on the trend, not a precise correctness gate.

print()
print("All test_helmholtz checks passed.")
